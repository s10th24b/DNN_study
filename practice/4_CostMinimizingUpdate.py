...
# W := W - α(1/m) sigma[i=1 to m] (W*X - Y) * X
# 여기서 α(learning_rate)하고 그 뒷부분이 다 델타(세모모양), 즉 기울기이다.
# minimum에서 오른쪽 부분은 기울기가 +이다. 그러므로, W := W-Δ 시, Δ가 +이므로, W는 - 방향으로 움직인다.
# 그 반대로, 왼쪽 부분은 기울기가 - 이므로, W := W-Δ 시, Δ가 -이므로, W는 +방향으로 움직인다.
